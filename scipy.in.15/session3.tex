...
Out[]: <matplotlib.image.AxesImage object at 0xa0384cc>
  \end{lstlisting}
  \end{small}
\typ{imread} returns an array of shape (NX, NY, 4) which represents an image
of NX x NY pixels and 4 shades.\\
...
\frametitle{Slicing \& Striding Exercises}
  ...
  \item Crop the image to get only the bird
...
\begin{itemize}
\item We need to fit a line through points for the equation $T^2 = m  L+c$

\item We have data in this form:\\
$ T_1^2 = m L_1 + c$\\
$ T_2^2 = m L_2 + c$\\
$ T_3^2 = m L_3 + c$\\
\ldots
\item Find ``best'' $m$ and $c$
\end{itemize}
\end{frame}
